...
_BOUNDING_BOX_ZORDER: Final[
    int
] = 15000  # Ensure actor bounding boxes are plotted on top of all map elements
...
def visualize_scenario_prediction(
    ...
    if traj_visible['gt']: #Overlay predictions - if they are off, the ground truth future will stick out regardless of zorder
        gt_eval_world = additional_traj['gt']
        for k in range(gt_eval_world.shape[0]):
            if k in special_set:
                plt.plot(gt_eval_world[k,:,0],gt_eval_world[k,:,1],color = 'black',linewidth = 6, zorder = 15000)
            else:
                plt.plot(gt_eval_world[k,:,0],gt_eval_world[k,:,1],color = 'black',linewidth = 6, zorder = 15000)



    if traj_visible['gt_his']:
        gt_his_eval_world = additional_traj['gt_his']
        for k in range(gt_eval_world.shape[0]):
            if k in special_set:
                plt.plot(gt_his_eval_world[k, :, 0], gt_his_eval_world[k, :, 1], color='orange', linewidth=4, zorder=20000)
            else:
                plt.plot(gt_his_eval_world[k, :, 0], gt_his_eval_world[k, :, 1], color='orange', linewidth=4, zorder=20000)
    ...
    if traj_visible['rec_traj']:
        ...
        color_set = ['dodgerblue', 'springgreen', 'orangered', 'blueviolet', 'brown', 'deeppink']
        for k in range(rec_traj.shape[0]):
            if k in special_set:
                for i in range(rec_traj.shape[1]):
                    plt.plot(rec_traj[k,i,:,0],rec_traj[k,i,:,1],color = color_set[i],linewidth = 4,alpha = 0.5, zorder = 20000)
                #plt.plot(rec_traj[k, min_fde_index, :, 0], rec_traj[k, min_fde_index, :, 1], color='purple', linewidth=6, alpha=0.8,
                #         zorder=10000)
            else:
                for i in range(rec_traj.shape[1]):
                    plt.plot(rec_traj[k,i,:,0],rec_traj[k,i,:,1],color = 'orange',linewidth = 4,alpha = 0.01, zorder = 20000)
